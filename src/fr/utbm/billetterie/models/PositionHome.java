...
public class PositionHome {
	...
	public Position merge(Position detachedInstance) {
		...
		try {
			Position result = (Position) sessionFactory.getCurrentSession()
	...
	public Position findById(PositionId id) {
		...
		try {
			Position instance = (Position) sessionFactory.getCurrentSession()
					.get("test.Position", id);
			if (instance == null) {
